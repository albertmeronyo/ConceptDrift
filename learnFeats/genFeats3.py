...
for ds in datasets:
    ...
    with open('feats_' + ds + '.csv', 'wb') as csvfile:
        ...
        for key in tree_o:
            ...
            if key in tree:
                if countChildren(tree, key, 0) == countChildren(tree_o, key, 0):
                    changed = 0
                elif countChildren(tree, key, 0) > countChildren(tree_o, key, 0):
                    changed = 1
                else:
                    changed = 2
            else:
...
with open('feats_3.9.csv', 'wb') as csvfile:
    ...
    for key in tree:
        ...
        # Target classes: 0 stable, 1 extended, 2 shrink, 3 lost
        if key in tree_o:
            if countChildren(tree_o, key, 0) == countChildren(tree, key, 0):
                changed = 0
            elif countChildren(tree_o, key, 0) > countChildren(tree, key, 0):
                changed = 1
            else:
                changed = 2
        else:
