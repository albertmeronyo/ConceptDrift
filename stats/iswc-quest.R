...
q2 <- paste(sparql_prefix2,
            "SELECT DISTINCT (str(?age_s) AS ?age_c) (str(?gender_s) AS ?gender_c) (str(?marital_status_s) AS ?marital_status_c) (str(?municipality_s) AS ?municipality_c) (str(?occupation_s) AS ?occupation_c) (str(?position_s) AS ?position_c) ?population
...
h1 <- read.xls('/Users/Albert/Downloads/index/1889_08_T1.xls')
h1$ID <- NULL
h1$ALTBEROEP <- NULL 
h1$STATUS <- NULL 
h1$REL <- NULL
...
# Load HISCO data and merge
h2 <- read.xls('/Users/Albert/Downloads/index2/1899_04_T.xls')
h2$ID <- NULL
h2$ALTBEROEP <- NULL 
h2$STATUS <- NULL 
h2$REL <- NULL
