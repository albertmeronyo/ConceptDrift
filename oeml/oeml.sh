...
python genFeatsP.py -i $DATA_DIR -o $EXP_DIR -n $NUM_SNAPS -dtt $DELTA_TT -dfc $DELTA_FC -c $CHANGE -t $TOP_CAT -s $SUB_PROP -m $IN_PROP -f nt 2> $EXP_DIR/error.txt > $EXP_DIR/feats_out.txt
