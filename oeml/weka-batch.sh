...
COMMANDS=( 
    ...
    "$JAVA_PREFIX weka.classifiers.bayes.BayesNet $USE_MODEL $TEST -i >> $OUTPUT" 
    "$JAVA_PREFIX_FS weka.classifiers.bayes.NaiveBayes >> $OUTPUT" 
    "$JAVA_PREFIX weka.classifiers.bayes.NaiveBayes $USE_MODEL $TEST -i >> $OUTPUT" 
    "$JAVA_PREFIX_FS weka.classifiers.bayes.NaiveBayesMultinomial >> $OUTPUT"
    "$JAVA_PREFIX weka.classifiers.bayes.NaiveBayesMultinomial $USE_MODEL $TEST -i >> $OUTPUT"
    "$JAVA_PREFIX_FS weka.classifiers.bayes.NaiveBayesMultinomialUpdateable >> $OUTPUT"
    "$JAVA_PREFIX weka.classifiers.bayes.NaiveBayesMultinomialUpdateable $USE_MODEL $TEST -i >> $OUTPUT"
    "$JAVA_PREFIX_FS weka.classifiers.bayes.NaiveBayesUpdateable >> $OUTPUT"
    "$JAVA_PREFIX weka.classifiers.bayes.NaiveBayesUpdateable $USE_MODEL $TEST -i >> $OUTPUT"
    "$JAVA_PREFIX_FS weka.classifiers.functions.Logistic -R 1.0E-8 -M -1 >> $OUTPUT"
    "$JAVA_PREFIX weka.classifiers.functions.Logistic $USE_MODEL $TEST -i >> $OUTPUT"
    "$JAVA_PREFIX_FS weka.classifiers.functions.MultilayerPerceptron -L 0.3 -M 0.2 -N 500 -V 0 -S 0 -E 20 -H a >> $OUTPUT"
    "$JAVA_PREFIX weka.classifiers.functions.MultilayerPerceptron $USE_MODEL $TEST -i >> $OUTPUT"
    "$JAVA_PREFIX_FS weka.classifiers.functions.SGD -F 0 -L 0.01 -R 1.0E-4 -E 500 -C 0.001 >> $OUTPUT"
    "$JAVA_PREFIX weka.classifiers.functions.SGD $USE_MODEL $TEST -i >> $OUTPUT"
    "$JAVA_PREFIX_FS weka.classifiers.functions.SimpleLogistic -I 0 -M 500 -H 50 -W 0.0 >> $OUTPUT"
    "$JAVA_PREFIX weka.classifiers.functions.SimpleLogistic $USE_MODEL $TEST -i >> $OUTPUT"
    "$JAVA_PREFIX_FS weka.classifiers.functions.SMO -C 1.0 -L 0.001 -P 1.0E-12 -N 0 -V -1 -W 1 -K \"weka.classifiers.functions.supportVector.PolyKernel -C 250007 -E 1.0\" >> $OUTPUT"
    "$JAVA_PREFIX weka.classifiers.functions.SMO $USE_MODEL $TEST -i >> $OUTPUT"
    "$JAVA_PREFIX_FS weka.classifiers.functions.VotedPerceptron -I 1 -E 1.0 -S 1 -M 10000 >> $OUTPUT"
    "$JAVA_PREFIX weka.classifiers.functions.VotedPerceptron $USE_MODEL $TEST -i >> $OUTPUT"
    "$JAVA_PREFIX_FS weka.classifiers.rules.DecisionTable -X 1 -S \"weka.attributeSelection.BestFirst -D 1 -N 5\" >> $OUTPUT"
    "$JAVA_PREFIX weka.classifiers.rules.DecisionTable $USE_MODEL $TEST -i >> $OUTPUT"
    "$JAVA_PREFIX_FS weka.classifiers.rules.JRip -F 3 -N 2.0 -O 2 -S 1 >> $OUTPUT"
    "$JAVA_PREFIX weka.classifiers.rules.JRip $USE_MODEL $TEST -i >> $OUTPUT"
    "$JAVA_PREFIX_FS weka.classifiers.rules.OneR -B 6 >> $OUTPUT"
    "$JAVA_PREFIX weka.classifiers.rules.OneR $USE_MODEL $TEST -i >> $OUTPUT"
    "$JAVA_PREFIX_FS weka.classifiers.rules.ZeroR >> $OUTPUT"
    "$JAVA_PREFIX weka.classifiers.rules.ZeroR $USE_MODEL $TEST -i >> $OUTPUT"
    "$JAVA_PREFIX_FS weka.classifiers.trees.DecisionStump >> $OUTPUT"
    "$JAVA_PREFIX weka.classifiers.trees.DecisionStump $USE_MODEL $TEST -i >> $OUTPUT"
    "$JAVA_PREFIX_FS weka.classifiers.trees.HoeffdingTree -L 2 -S 1 -E 1.0E-7 -H 0.05 -M 0.01 -G 200.0 -N 0.0 >> $OUTPUT"
    "$JAVA_PREFIX weka.classifiers.trees.HoeffdingTree $USE_MODEL $TEST -i >> $OUTPUT"
    "$JAVA_PREFIX_FS weka.classifiers.trees.J48 -C 0.25 -M 2 >> $OUTPUT"
    "$JAVA_PREFIX weka.classifiers.trees.J48 $USE_MODEL $TEST -i >> $OUTPUT"
    "$JAVA_PREFIX_FS weka.classifiers.trees.RandomForest -I 10 -K 0 -S 1 -num-slots 1 >> $OUTPUT"
    "$JAVA_PREFIX weka.classifiers.trees.RandomForest $USE_MODEL $TEST -i >> $OUTPUT"
    "$JAVA_PREFIX_FS weka.classifiers.trees.RandomTree -K 0 -M 1.0 -S 1 >> $OUTPUT"
    "$JAVA_PREFIX weka.classifiers.trees.RandomTree $USE_MODEL $TEST -i >> $OUTPUT"
    "$JAVA_PREFIX_FS weka.classifiers.trees.REPTree -M 2 -V 0.001 -N 3 -S 1 -L -1 -I 0.0 >> $OUTPUT"
    "$JAVA_PREFIX weka.classifiers.trees.REPTree $USE_MODEL $TEST -i >> $OUTPUT"
)
